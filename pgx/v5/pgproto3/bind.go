...
func (dst *BindEx) Decode(src []byte) error {
	*dst = BindEx{}
	...
	payloadCount := int(binary.BigEndian.Uint16(src[rp:]))
	rp += 2
	if payloadCount > 0 {
		dst.PtagToPayload = make(map[string]*PayloadBuffer, payloadCount)
	}
	...
	for i := 0; i < payloadCount; i++ {
		...
		ptag := string(src[rp : rp+idx])
		rp += idx + 1

		if len(src[rp:]) < 4 {
			return &invalidMessageFormatErr{messageType: "BindEx"}
		}
		payloadLen := int(binary.BigEndian.Uint32(src[rp:]))
		rp += 4
		if payloadLen < 0 || len(src[rp:]) < payloadLen {
			return &invalidMessageFormatErr{messageType: "BindEx"}
		}
		payloadData := make([]byte, payloadLen)
		copy(payloadData, src[rp:rp+payloadLen])
		rp += payloadLen

		dst.PtagToPayload[ptag] = &PayloadBuffer{
			Data: payloadData,
			Tail: payloadLen,
			Cap:  payloadLen,
		}
