...
func (i *IoT) LastLocByTruck(qi query.Query, nTrucks int) {
	...
	var sql string
	if !prepare {
		sql = fmt.Sprintf(`SELECT last(k_timestamp), last(latitude), last(longitude) FROM %s.public.readings WHERE %s GROUP BY name`,
			i.ReadingDBName,
...
func (i *IoT) LastLocPerTruck(qi query.Query) {
	...
	var sql string
	if !prepare {
		sql = fmt.Sprintf(`SELECT last(k_timestamp), name, last(driver), last(latitude) as latitude, last(longitude) as longitude FROM %s.public.readings WHERE fleet='%s' and name IS NOT NULL GROUP BY name`,
...
func (i *IoT) TrucksWithLowFuel(qi query.Query) {
	...
	var sql string
	if !prepare {
		sql = fmt.Sprintf(`SELECT last_row(k_timestamp) ,name, last(driver) as driver ,last(fuel_state) as fuel_state FROM %s.public.diagnostics where fleet = '%s' and fuel_state <= 0.1   GROUP BY name`,
			i.ReadingDBName, i.GetRandomFleet())
	} else {
		sql = fmt.Sprintf(`%s`, i.GetRandomFleet())
	}
...
func (i *IoT) AvgVsProjectedFuelConsumption(qi query.Query) {
	sql := fmt.Sprintf("select avg(fuel_consumption) as avg_fuel_consumption,avg(nominal_fuel_consumption) as nominal_fuel_consumption from %s.public.readings where velocity > 1 group by fleet", i.ReadingDBName)
...
func (i *IoT) AvgDailyDrivingSession(qi query.Query) {
	...
	if !prepare {
        ...
        FROM (SELECT time_bucket(k_timestamp, '600s') as ts,name,diff(cast(cast(floor(avg(velocity)/5) as bool) as int))  
     	OVER (PARTITION BY name order by time_bucket(k_timestamp, '600s'))  AS difka FROM %s.public.readings   
...
func (i *IoT) AvgLoad(qi query.Query) {
	sql := fmt.Sprintf("select fleet,model,load_capacity,avg(ml/load_capacity)  from (SELECT last(fleet) as fleet, last(model) as model,last(load_capacity) as load_capacity,avg(current_load) AS ml FROM %s.public.diagnostics  group BY name) group BY fleet, model,load_capacity", i.ReadingDBName)
...
func (i *IoT) TruckBreakdownFrequency(qi query.Query) {
	...
	if !prepare {
		...
		AS broken_down  FROM %s.public.diagnostics where k_timestamp  > '%s' AND k_timestamp  <= '%s' group by name,time_bucket(k_timestamp, '600s'))) WHERE state_changed = 1 group by model`,
			i.DiagnosticsDBName,
