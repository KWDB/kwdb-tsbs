...
func (p *processor) InitLastLoc() {
	...
	p.prepareStmt.WriteString("SELECT last(k_timestamp), name, last(driver), last(latitude) as latitude, last(longitude) as longitude FROM benchmark.public.readings WHERE fleet=$1 and name IS NOT NULL GROUP BY name")
...
func (p *processor) InitLowFuel() {
	p.prepareStmt.Grow(150)
	p.prepareStmt.WriteString("SELECT last(k_timestamp), last(latitude), last(longitude) FROM benchmark.public.readings WHERE name IN ($1) GROUP BY name")
	p.formatBuf = make([]int16, 1)
	p.formatBuf[0] = 1
}
...
func (p *processor) InitSingleLastLoc() {
	p.prepareStmt.Grow(150)
	p.prepareStmt.WriteString("SELECT last_row(k_timestamp) ,name, last(driver) as driver ,last(fuel_state) as fuel_state FROM benchmark.public.diagnostics where fleet = $1 and fuel_state <= 0.1   GROUP BY name")
	p.formatBuf = make([]int16, 1)
	p.formatBuf[0] = 1
}
...
func (p *processor) InitStationaryTrucks() {
	p.prepareStmt.Grow(150)
	p.prepareStmt.WriteString("select name,driver from (SELECT name,driver,avg(velocity) as mean_velocity from benchmark.public.readings  where fleet =$1 and k_timestamp > $2 AND k_timestamp <= $3 group BY name,driver) WHERE mean_velocity < 1")
...
func (p *processor) InitLongDrivingSessions() {
	...
	p.prepareStmt.WriteString("SELECT name,driver FROM (SELECT name,last(driver) as driver,avg(velocity) as mean_velocity FROM benchmark.public.readings WHERE fleet = $1 AND k_timestamp > $2 AND k_timestamp <= $3 group BY name, time_bucket(k_timestamp, '600s')) WHERE mean_velocity > 1 GROUP BY name,driver having count(*) > 22")
	p.formatBuf = make([]int16, 3)
	for i := 0; i < 3; i++ {
		p.formatBuf[i] = 1
	}
...
func (p *processor) InitLongDailySessions() {
	p.prepareStmt.Grow(300)
	p.prepareStmt.WriteString("SELECT name,last(driver) as driver FROM benchmark.public.readings WHERE fleet =$1 AND k_timestamp > $2 AND k_timestamp <= $3 group BY name having count(*) > 60 and avg(velocity) > 1 ")
	p.formatBuf = make([]int16, 3)
	for i := 0; i < 3; i++ {
		p.formatBuf[i] = 1
	}
...
func (p *processor) InitConsumption() {
	p.prepareStmt.Grow(300)
	p.prepareStmt.WriteString("select avg(fuel_consumption) as avg_fuel_consumption,avg(nominal_fuel_consumption) as nominal_fuel_consumption from benchmark.public.readings where velocity > 1 group by fleet")
}
...
func (p *processor) InitDuration() {
	...
	p.prepareStmt.WriteString("select name,min(fleet) as fleet,min(driver) as driver,avg(hours_driven) as avg_daily_hours from (select time_bucket(k_timestamp, '1d') as " +
		...
		"as k_timestamp,name,last(fleet) as fleet ,last(driver) as driver,avg(velocity) as avg_v from benchmark.public.readings where " +
...
func (p *processor) InitSession() {
	...
	p.prepareStmt.WriteString(`select time_bucket(ts, '1d') as ts,name,avg(ela) from (SELECT ts,name,diff(difka)  
        ...
        FROM (SELECT time_bucket(k_timestamp, '600s') as ts,name,diff(cast(cast(floor(avg(velocity)/5) as bool) as int))  
     	OVER (PARTITION BY name order by time_bucket(k_timestamp, '600s'))  AS difka FROM benchmark.public.readings   
...
func (p *processor) InitLoad() {
	p.prepareStmt.Grow(300)
	p.prepareStmt.WriteString("select fleet,model,load_capacity,avg(ml/load_capacity)  from (SELECT last(fleet) as fleet, last(model) as model,last(load_capacity) as load_capacity,avg(current_load) AS ml FROM benchmark.public.diagnostics  group BY name) group BY fleet, model,load_capacity")
}
...
func (p *processor) InitActivity() {
	...
	p.prepareStmt.WriteString("SELECT time_bucket(k_timestamp, '1d') as k_timestamp, model,fleet,count(avg_status)/144 FROM" +
		...
		" avg_status FROM benchmark.public.diagnostics WHERE k_timestamp >= $1 AND k_timestamp < $2 group by  name,time_bucket(k_timestamp, '600s'))" +
...
func (p *processor) InitFrequency() {
	...
	p.prepareStmt.WriteString(`SELECT model,count(state_changed) FROM (SELECT model,diff(broken_down)  OVER (PARTITION BY name order by ts)  
		...
		AS broken_down  FROM benchmark.public.diagnostics where k_timestamp  > $1 AND k_timestamp  <= $2 group by name,time_bucket(k_timestamp, '600s'))) WHERE state_changed = 1 group by model`)
	p.formatBuf = make([]int16, 2)
	for i := 0; i < 2; i++ {
		p.formatBuf[i] = 1
	}
