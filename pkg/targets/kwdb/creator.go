...
func (d *dbCreator) CreateDB(dbName string) error {
	...
	if d.opts.Case == "cpu-only" {
		...
		if d.opts.Partition {
			...
			sqlpartition = fmt.Sprintf("ALTER PARTITION p0 OF TABLE %s.cpu CONFIGURE ZONE USING lease_preferences = '[[+region=NODE1]]',constraints = '{\"+region=NODE1\":1}',num_replicas=3 SYNC;"+
				"ALTER PARTITION p1 OF TABLE %s.cpu CONFIGURE ZONE USING lease_preferences = '[[+region=NODE2]]',constraints = '{\"+region=NODE2\":1}',num_replicas=3 SYNC;"+
				"ALTER PARTITION p2 OF TABLE %s.cpu CONFIGURE ZONE USING lease_preferences = '[[+region=NODE3]]',constraints = '{\"+region=NODE3\":1}',num_replicas=3 SYNC;", dbName, dbName, dbName)
			_, err = d.db.Connection.Exec(ctx, sqlpartition)
			if err != nil {
